...
## 떠 있는 창 배치 넷을 뽑는다.
##
##     godot --path . -s res://tools/capture_panes.gd -- .renders/33-panes still
##     godot --path . -s res://tools/capture_panes.gd -- .renders/pan film
##     python tools/make_gif.py .renders/pan .renders/33-panes.gif 0.34
##
## **창 은유는 정지로는 판정이 안 된다.** 앞뒤가 바뀌는 것이 이 배치의 사건이라
## GIF 가 판정 매체고 정지는 그 도중 한 장이다.

const FPS: float = 20.0

## 태블릿 넷. 판 하나가 1280x720 이라 **창 크기가 거짓말을 안 한다.**
const SHEET := Vector2i(2620, 1730)

## 뒤 창이 앞으로 다 나온 순간.
const STILL_AT: float = 2.60
const SETTLE := 8

var _prefix := "res://.renders/panes"
var _film := false
var _stage: SubViewport
var _bench: PaneStage
var _waited := 0
var _saved := 0
var _posed := -1
...
func _initialize() -> void:
	...
	_bench = load("res://src/ui/kit/pane_stage.tscn").instantiate() as PaneStage
...
func _process(_delta: float) -> bool:
	...
	_saved += 1
	if _saved < int(PaneStage.LOOP * FPS):
		return false
