...
## **다 재고 나면 스스로 끝난다.**
##
## 안 끝나서 사고가 났다 — 표를 다 뽑고도 **창이 남아 GPU 를 100 % 물고 있었다.**
## VRAM 14.4 / 16.4 GB 를 쥔 채였고, 사람이 손으로 꺼야 했다.
##
## **재는 도구가 잴 일이 없으면 남아 있을 이유가 없다.** 남아 있는 것은 결과가 아니라
## 점유다 — 화면에 띄워 두는 것이 값이 될 때는 **사람이 보고 있을 때뿐**이다.
##
## **웹에서는 안 끝낸다.** 브라우저에서 끝내면 표를 읽기 전에 화면이 사라진다.
## 대신 재는 것을 멈춘다 — 대원을 이미 다 놓았으므로 그것만으로 거의 빈 화면이 된다.
func _stop() -> void:
	set_process(false)
