...
화풍에서 파노라마 12~13% · 벽 타일 22.8% · 바닥 타일 1.1% 다. 화풍 후보 넷에
대 보니 `bleach_deep` 만 벽 타일 **64.1%** 로 튀었고 나머지 셋은 현행과 같은
띠였다 (`26-2d-lighting.md` §26.4.13.2).

**그리고 굽고 나서 이 도구가 찍는 바닥 통계를 읽어라.** 네 판(사실적 둘 +
화풍 후보 둘)에서 바닥이 **27~30%**, 고치기 전 `N.y` **0.93~0.99**, 고친 뒤
`N.z` **0.98 이상**으로 붙어 있었다. 무너지면 `_FLOOR_FROM` 이 그 재질에
안 맞는 것이다 — 다만 가장 평평한 후보(`bleach_deep`)에서도 안 무너졌다.
