...
##
## **280 에서 240 으로 내렸다.** 방 50개짜리 판이 확대 하한에서 화면에 들어오지 않았다 —
## 전체를 담으려면 배율 0.373 이 필요한데 하한이 0.40 이다. 방 32개까지는 들어왔다.
##
## 플레이 배율에서 판이 화면을 넘는 것은 의도지만(위 주석), **훑어보는 배율에서 넘는 것은
## 결함이다.** 낮은 배율은 "지금 판이 어떻게 생겼나"를 보는 배율이라고
## docs/design/07-level-design.md §7.8 이 따로 정해 뒀고, 판이 잘려 있으면 그게 성립하지 않는다.
##
## 240 이면 필요 배율이 0.432 가 되어 담기고, 가장 가까운 두 방의 위젯 사이가 58 px 남는다
## (위젯 너비 146 의 40%). **220 아래로는 내리지 않는다** — 44 px 로 줄어 이름이 붙어 보인다.
## 실측은 docs/design/17-dungeon-generation.md §17.15, 도구는 tools/check_board_capacity.gd.
const _SPACING := 240.0
