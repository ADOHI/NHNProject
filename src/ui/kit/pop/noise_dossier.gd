...
##
## ⚠ **이 비율은 그림용이지 실측이 아니다.** 겪어서 아는 얼굴 3/20 · 들어 본 이름 5/20 은
## **세 단이 한 장에 다 나오게** 고른 값이다. 실제 목록은 이보다 훨씬 성기다 — 인구
## 전체로 보면 대원과 겪어서 아는 사람은 1000명에 하나쯤이고 들어 본 이름도 스물에
## 하나쯤이라, 진짜 값을 물으면 **스무 줄이 거의 다 잡음**으로 나온다.
##
## 그것은 회귀가 아니라 **맞는 그림**이다. 이 축이 하려던 말이 원래 그것이다 —
## 길드원 목록을 열면 대개 모르는 사람이고, 아는 얼굴 하나가 그 사이에서 혼자 맑다.
##
