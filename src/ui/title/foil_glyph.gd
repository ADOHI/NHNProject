...
##
## 이 값을 두 번 틀렸다. 양쪽 실패를 다 적어 둔다.
##
## | 값 | 무슨 일이 났나 |
## | --- | --- |
## | 0.026 (≈5px) | 심사자 둘 다 **움직임을 아예 못 봤다.** 없는 것과 같았다 |
## | 0.105 (≈47px) | 움직임은 보이는데 **글자가 안 읽혔다.** 세 덩어리로 흩어졌다 |
##
## 두 번째가 더 나쁜 실패다. **제목 화면에서 제목을 못 읽으면 나머지는 의미가 없다.**
##
## 그리고 여기서 배운 것이 하나 있다 — 「금」은 명조에서 이미
## ㄱ · ㅡ · ㅁ 세 획이 **떨어져** 앉는다. 글자 자체가 이미 세 덩어리라
## 조각을 조금만 밀어도 곧바로 "부품 셋"으로 읽힌다.
## 다른 글자보다 어긋남에 훨씬 약하다.
##
## 그래서 **어긋남으로 균열을 말하지 않기로 했다.** 균열은 금 위에 그려진
## 검은 금이 말하고(`_CrackWeb`), 어긋남은 그 금이 **살아 있다는 표시**만 한다.
## 글자를 부수는 것이 아니라 **글자 위에 난 것**이어야 한다.
const _SLIP_REST := 0.032
const _SLIP_ALMOST := 0.004
...
## 머무는 동안 벌어진 폭.
##
## **틈은 획을 먹는다.** 벌어진 폭만큼 글자가 사라지므로, 이 값이 크면
## 가는 획이 통째로 끊겨 글자가 조각으로 흩어진다. 균열을 보여 주는 일은
## 틈이 아니라 **그려진 검은 금**이 한다. 여기서는 그 금에 두께만 준다.
const _OPEN_REST := 0.008

## 처음 눌러 찍을 때 벌어지는 폭. 박이 열과 압력에 맞는 순간이다.
##
## 이 한순간만 크게 벌어진다. 눌리는 순간은 읽히지 않아도 되지만
## **머무는 동안은 늘 읽혀야** 하기 때문이다.
const _OPEN_STRIKE := 0.070
