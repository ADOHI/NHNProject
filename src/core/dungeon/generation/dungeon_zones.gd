...
##
## 0.38 이면 여유가 8.9 px 이다. **값을 올려도 판은 거의 안 변한다** —
## 방 50개에서 0.35 / 0.38 / 0.42 가 순환 17.4~17.5, 지름 15.1~15.5, 차수 2.71 로
## 사실상 같다. 0.38 을 고른 것은 **공짜로 여유를 두 배 넘게 벌기 때문**이다.
##
## 0.42 까지 올리지 않은 이유는 작은 판에서 값을 치르기 때문이다 —
## 방 32개에서 기본 민첩으로 닿는 방이 87.5% -> 84.7% 로 준다.
## 후보가 줄면 관문 자리가 좁아지고, 그 영향은 판이 작을수록 크다.
