...
## **그 대원을 그만큼 붙들어 둔다** — §28.4 의 **대기 키**를 아주 거칠게 흉내낸 것.
##
## `docs/design/28-combat.md` §28.20.59.
##
## 사용자 확정: *"RTS 기본으로 행동하되 대기 키를 만들어 두면 고도화된 컨트롤 가능."*
## 그 「고도화」가 실제로 무엇을 사는지가 안 재져 있었다.
##
## ## 무엇을 흉내내나 — **타가 나가는 시각 하나뿐이다**
##
## 진짜 대기 키는 이동도 표적도 같이 멈춘다. **여기서 흉내내는 것은 「언제 치기
## 시작하나」뿐**이고, 그것이 값을 하는 까닭은 무너짐 눈금이 **초당 감쇠**하기
## 때문이다 — 흩어 치면 문턱에 닿기 전에 빠진다 (§28.20.35).
##
## **`start()` 뒤에 부른다.** `start()` 가 붙든 것을 전부 푼다.
func hold(ally: int, seconds: float) -> void:
	if ally < 0 or ally >= _ally_offset.size() or seconds <= 0.0:
		return
	_ally_offset[ally] += seconds
