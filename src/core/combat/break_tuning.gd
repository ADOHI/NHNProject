...
## **걸침이 앞뒤로도 걸리나, 좌우로만 걸리나** (§28.20.38).
##
## 적이 한 줄로만 서던 때에는 물을 것이 없었다. 켜가 생기면서 갈린다 —
## 한 번 휘두른 것이 **같은 켜의 이웃**까지인가, **뒤 켜**까지 닿는가.
##
## **기본값은 `true` — 켜를 안 가린다.** 켜가 없던 때의 동작 그대로다.
## `false` 면 고른 적과 **같은 켜**만 함께 맞는다.
##
## ## 이것은 **판의 손잡이가 아니라 그 타의 마무리 자세**가 정할 것이다
##
## 처음에는 「무기 모양이 정할 것」으로 읽었는데 **틀렸다** (§28.20.42).
## 애니 레인 실측에 **무기는 자세도 궤적도 안 가른다** — 갈리는 것은 크기와 속도뿐이다.
##
## 동작은 **자세 쌍**(`from_guard -> to_guard`)에 있고, 그 쌍을 **부르는 쪽이 고른다.**
## 이으면 `CharActor.closing_guard()` 가 그 타의 마무리 자세를 주고
## 이 한 값이 **타마다** 갈린다 — 판 전체 하나가 아니다.
##
## > **누가 자세 쌍을 고르나는 아직 안 정해졌다** (§28.20.45).
## > 정해지기 전까지는 판 하나에 하나로 둔다.
var splash_spans_ranks: bool = true
