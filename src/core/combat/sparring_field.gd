...
## **적이 다가오는 속력의 기본값** (초당 픽셀). `approach_speed` 의 출고 값이다.
##
## ## 수를 지어내지 않았다 — 출처
##
## 이 수는 **이동 레인의 실제 유닛 속도**다 —
## `src/proto/unit_move/core/move_tuning.gd` 의 `ProtoMoveTuning.DEFS` 에서
## `max_speed` 항목의 `value`, 즉 **230**.
##
## 속력은 **계단**이라 평평한 쪽에서는 어느 수를 골라도 결과가 같은데
## (§28.20.53 — 40 · 230 · 400 이 다 90명), **그러면 고를 이유가 있는 수를 고른다**
## (§28.20.50 의 길 · §28.20.55 의 표).
##
## ## 왜 부르지 않고 베꼈나
##
## 전에는 `ProtoMoveTuning.new().default_of("max_speed")` 로 **불렀다.**
## 그러면 **출시되는 전투 규칙이 「버리려고 만든 프로토타입」에 매달린다** — 방향이
## 거꾸로고, 프로토를 출시 빌드에서 뺄 수가 없다 (`docs/refactoring-2026-08-19.md` §1.2).
##
## **값은 베끼되 관계는 안 버린다** — `test/unit/test_sparring_queue.gd` 가 이 상수와
## `ProtoMoveTuning` 의 기본값이 **같은지 본다.** 시험은 빌드에 안 들어가므로
## 거기서는 프로토를 불러도 된다. 이동 레인이 `max_speed` 를 옮기면 **그 시험이
## 빨개져서 알려 주고**, 그때 고칠 곳은 여기 한 줄이다 (§28.20.50 의 길 그대로).
const DEFAULT_APPROACH_SPEED := 230.0
...
##
## **§28.20.36~49 의 표가 전부 「아무도 안 빠진다」 위에 서 있었다.**
## 적이 여섯까지일 때는 그 차이가 작았지만 **물량전에서는 그것이 전부**다 —
## 앞줄이 안 비면 뒷줄과 싸울 일이 영영 없다.
##
## **2026-08-10 에 켜졌다** (§28.20.55). 사용자가 *"무리 만나면 한꺼번에 상대"* 로 정했고,
## **이것과 `approach_speed` 는 둘 다 있어야 뜻이 생긴다** — 앞줄이 안 비면 뒤가 들어올
## 자리가 없고, 자리가 비어도 아무도 안 걸어오면 그 자리는 계속 빈 채다.
## 하나만 켠 판은 둘 다 끈 판과 상한이 같다 (여섯).
var downed_leave: bool = true

## **적이 다가오는 속력** (초당 픽셀, §28.20.53).
##
## §28.20.52 가 물량전 상한을 **여섯**으로 재고 그 까닭을 「한 번에 몇이 닿나」로 짚었다.
## 그리고 그 절이 상한을 옮길 후보 넷 중 **이것이 제일 자연스럽다**고 적었다 —
## 그 절이 잰 것은 **계단**이다 — 0 과 0 아닌 것 사이에서 전부 갈리고 10 이든 400 이든 같다.
##
## **2026-08-10 에 켜졌다** (§28.20.55). 사용자가 *"전투 물량은 적이 다가오는 거"* 로 정했다.
##
## ## 수를 지어내지 않는다
##
## 기본값의 출처는 `DEFAULT_APPROACH_SPEED` 에 적어 뒀다 — **이동 레인의 실제 유닛
## 속도**이고, 그것이 옮겨지면 시험이 빨개진다 (§28.20.50 의 길).
##
## > 이것은 §28.4 의 RTS 자동 이동이 할 일의 **아주 거친 흉내**다 —
## > 길찾기도 대형도 막힘도 없이 **앞으로만** 온다. 그 자리를 이동 레인이 채우면 지워진다.
var approach_speed: float = DEFAULT_APPROACH_SPEED
...
func targets_within(reach: float, limit: int = 1) -> PackedInt32Array:
	...
	#
	# **비교기를 여기서 만들지 않는다.** 이 함수는 한 판에 수십만 번 불린다
	# (적 100 명 · 180 초). 그때마다 `Callable` 을 새로 지으면 그만큼 쓰레기가 생긴다.
	ordered.sort_custom(_nearer_first)
...
## `(거리, 번호)` 둘을 견준다. **가까운 것 먼저, 같으면 번호 먼저.**
static func _nearer_first(a: Vector2, b: Vector2) -> bool:
