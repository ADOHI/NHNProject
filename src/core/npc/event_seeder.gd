...
## 사건 종류의 상대 가중치. **`RelationEvent.Kind` 순서와 나란하다.**
##
## **협력이 압도적으로 흔해야 한다.** 배신이 흔하면 배신이 특별하지 않고,
## 설계 24.5 가 *"배신이 이 시스템의 핵심"* 이라 한 것이 값을 잃는다.
##
## **후유증은 0 이다** — 세계가 굴리는 사건이 아니라 내 원정이 남기는 것이라
## `ExpeditionAftermath` 가 적는다. 자리를 비워 두면 enum 과 어긋난다.
##
## 구조와 정보는 **호감을 되돌리는 쪽**이다. 올리는 길이 협력 하나뿐이면
## 세계가 400판에 걸쳐 한 방향으로만 어두워진다 (설계 24.31).
##
## **이 값들은 실측으로 고른 것이고 베끼면 안 된다** — 사건을 붙일 때마다 400판 곡선을
## 다시 재고 되맞춘다 (설계 24.38.3). 지금 값이 400판 세계 호감 16.6 을 낸다.
##
## **거짓 정보와 밀고를 낮게 둔 이유**는 둘이 세계를 가장 빨리 어둡게
## 만들기 때문이다 — 각각 20% 로 올리면 400판 호감이 8.6 · 10.2 로 떨어진다 (§24.36.3).
##
## **대상 없는 사건 여섯의 몫을 10 으로 묶어 둔 것도 실측이다** (§24.38.3) — 호감은
## 거의 안 내리는데 **대원이 좋은 일의 대상이 될 자리를 줄여** 회복 경로가 마른다.
const KIND_WEIGHTS := [19, 6, 3, 0, 15, 18, 5, 2, 2, 4, 3, 2, 5, 2, 2, 2, 2, 2, 2, 2, 2]
