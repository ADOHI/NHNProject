...
## 세계를 한 틱 돌린다 — **던전 1회 = 세계 1틱** (설계 15.1 · 24.11).
##
## *"매 틱 세계 곳곳에서 사건이 발생하고, 연루자들의 관계가 각자 성향대로 변한다.
## 대부분은 플레이어가 모른다."*
##
## **이것이 없으면 판을 돌려도 세계가 그대로다.** 실제로 그 상태를 재 봤고,
## 여든 판을 돌려도 관계 수가 4069 에서 4088 로 끝이었다 — 플레이어 길드 다섯 명만
## 움직이고 나머지 2995명은 가만히 있었다 (§24.30.5).
func tick(events: int) -> void:
	if _events == null or events <= 0:
		return
	_events.roll(events)
