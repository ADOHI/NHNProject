...
## 칸 하나의 화면 세로 폭(픽셀). **가로의 1/2 — 2:1 dimetric 이다.**
##
## ## 이 값은 그림에서 나왔다 (§30.9 ★1 — 확정)
##
## 마름모 모서리 기울기 **26.57도**(`atan 1/2`), 내려보는 각 **30도**(`asin 1/2`).
## 건물 그림이 이 각도로 확정됐다 (LANE-RULES §5 — `dimetric 2:1 · 26.57도`).
##
## ## 한 번 3:1 로 갔다가 되돌아왔다 — 되돌린 근거를 남긴다
##
## 처음에는 인물 그림(`pp1_*`)의 접지 그림자와 장화 윗면을 보고 3:1(19.47도)로 세웠다.
## **그 근거가 틀렸다.** `pp1_*` 는 **전신 캐릭터 시트**이고, 시트는 게임 카메라와 무관하게
## 정면으로 그리는 것이 보통이다 — **피치의 증거가 되지 못한다.**
##
## 되돌린 결정적인 이유는 따로 있다. 배회를 세워 **사람을 판에 실제로 세워 보니**
## 3:1 에서는 「제대로 된 층 높이」와 「2층」을 **동시에 못 가졌다** — 2층이 판의 43% 를
## 가렸다. 2:1 에서는 같은 층 높이에 21% 다 (docs/design/30-hideout.md §30.12.2).
##
## **실물이 서기 전에는 답할 수 없는 물음이었다.**
##
...
##
## 유도: 내려보는 각 `t = asin(세로/가로)`. 칸 한 변의 월드 길이는 `반폭 x sqrt(2)` 이고,
## 세로는 거기에 `cos(t)` 가 곱해진다. 2:1 에서는 **58.79 px** 이다.
##
## **상수로 박지 않는다.** 3:1 일 때 이 값이 정확히 64.0 이라 상수로 두었는데,
## 비율을 2:1 로 바꾸는 순간 그 64 가 **혼자 낡은 값**이 되었다.
## 비율에서 유도하면 비율만 고쳐도 따라온다.
static func cell_height_px() -> float:
	var pitch := asin(DEFAULT_TILE_HEIGHT / DEFAULT_TILE_WIDTH)
	return DEFAULT_TILE_WIDTH * 0.5 * sqrt(2.0) * cos(pitch)
