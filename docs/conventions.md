...
4. **어떤 조건에서 쟀는지도 적는다.** 명령만으로는 다시 안 나오는 값이 있다 —
   기계가 조용했는지, 창이 실제로 보이고 있었는지, 어느 빌드였는지.
   **재는 환경이 물음을 통째로 무효로 만들 수 있다** (아래)

> **측정 환경이 대상을 멈춰 세울 수 있다.**
> Godot 웹 빌드는 메인 루프가 `requestAnimationFrame` 이라 **문서가 숨겨져 있으면
> 한 프레임도 안 돈다.** 그런데 부팅 로그는 멀쩡히 찍혀서 **화면이 잘 뜬 것처럼 보인다.**
> 얼어붙은 채로 어떤 수를 읽어 적었으면 「웹이 이렇게 느리다」로 박혔을 것이다
> (`design/17-dungeon-generation.md` §17.21.6).
>
> **재기 전에 「재는 대상이 실제로 돌고 있나」를 먼저 확인해라.** 진행 표시가 올라가는지,
> 프레임이 도는지. **결과가 그럴듯한 것은 돌고 있다는 증거가 아니다.**
...
> | **한꺼번에 보여 주는 것이 쓸 만한가** | **묶음마다 세라** — 낱개가 1% 여도 묶음은 71% 가 될 수 있다 |
> | 대개 몇 개쯤 나오나 | 평균 또는 중앙값 |
>
> **마지막에서 둘째가 실제로 물렸다.** 게이트 하나가 막힐 확률은 1% 였는데
> **게이트 넷이 다 막힌 목록**은 71.5% 였다 — 넷이 독립이 아니라 같은 원인
> (단차 기본값)이 넷을 한꺼번에 민 것이다. **플레이어가 보는 단위로 세야 한다**
> (`design/17-dungeon-generation.md` §17.42.1).
>
