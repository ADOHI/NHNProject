...
### 23.11.6 처방 — **예산을 두 입구가 나눠 쓰고, 기록을 되돌리지 않는다**

두 줄이다. 새 층도, 새 상태도, 새 상수도 없다 — **이미 있던 예산을 빠진 입구에 마저 걸었다.**

| 어디 | 무엇 |
| --- | --- |
| `unit_agent.gd` `resume()` | 최단 기록(`creep_best`·`best_distance`)을 **지금 거리로 되돌리지 않는다.** `minf` 로 더 좋은 쪽을 남긴다. 시계는 그대로 되돌린다 |
| `unit_field.gd` `_review_moving` | 눌림·정체로 기다림에 떨어질 때 **`hold_retries` 를 쓴다.** 다 쓰면 `STOPPED` 로 그만둔다 |
| `unit_field.gd` `_review_moving` | **여태껏보다 가까워지면 `hold_retries` 를 0 으로 되돌린다** |

**되돌리는 조건이 시간이 아니라 진전이라는 것이 핵심이다.** 줄을 서서 한 몸씩 나아가는
유닛은 나아갈 때마다 예산을 되돌려 받으므로 여기 안 걸린다 — 걸리는 것은 **제자리에서
맴도는 유닛**뿐이다. 그리고 `creep_best` 는 이제 **줄기만 하는 기록**이라 되돌림의 횟수도
「처음 거리 나누기 0.5」로 묶인다. **그래서 유한성이 증명된다** —
기다림 횟수 ≤ 예산 × (되돌림 횟수 + 1) 이고 둘 다 유한하다.

#### 고친 뒤

앞서 안 멎던 **16 판을 그대로 다시 굴렸다. 16 판 모두 멎는다.**

| | 고치기 전 | 고친 뒤 |
| --- | --- | --- |
| 안 멎은 판 | **16 / 298** | **0 / 162** |
| 최장 최종정지 | 안 멎음 (300 초까지 확인) | **25.92 초** |

#### 남은 꼬리 — **낀 유닛 하나가 예산을 태우는 20 초**

고친 뒤 가장 오래 걸린 판(씨앗 140)을 초마다 찍었다. 값이 한 유닛에 다 들어 있다.

```
TRACE  6.0 양보 6  16(d39.3 c37  g37  p0 b0 h0 자리4@31.0)
TRACE 14.0 양보 4  16(d39.3 c16  g16  p0 b0 h1 자리4@31.0)
TRACE 18.0 양보 3  16(d40.1 c10  g10  p0 b0 h2 자리4@31.0)
TRACE 22.0 양보 3  16(d44.1 c4   g4   p0 b0 h3 자리4@31.0)
TRACE 26.0 양보 3
```

**`d` 가 소수점까지 얼어 있다.** 유닛 16 은 자리에서 39.3 px 떨어진 채 **한 발도 못 움직인다.**
그런데 `b0`(아무도 안 막았다) · `p0`(눌리지도 않았다) · `자리4@31.0`(제 자리를 남이 차지한
것도 아니다)이다.

**옆에 낀 것이다.** `unit_step.gd` 의 `room` 은 이웃이 **정면(`AHEAD_CONE`)** 에 있을 때만
`pressed`/`blocker_id` 를 적는다. 비스듬히 낀 몸은 빈 거리를 0 으로 깎으면서도 아무 표시를
안 남기므로, 이 유닛은 **어느 「누가 막았나」 그물에도 안 걸린 채** 정체 시계만 돈다.

그래서 한 번 낄 때마다 **4 초(`_CREEP_STALLED_FRAMES`)** 를 쓰고, 예산 셋에 진전 판정으로
돌려받은 둘을 더해 **다섯 번**을 돌아 25.92 초에 멎었다. **멎기는 멎는다** — 그것이 이번에
고친 성질이다. 다만 그 값이 **시험 한도 30 초에 아주 가깝다.**

> **여기서 이동 규칙을 더 건드리지 않는다.** 「비스듬히 낀 몸을 누가 막았나로 셀 것인가」는
> `AHEAD_CONE` 과 걸음 고르기를 함께 움직이는 문제라 이 레인의 범위(시험 하나를 정직하게
> 초록으로 만들기)를 넘는다. **다음에 꺼낼 것으로 여기 적어 둔다.**

### 23.11.7 시험을 어떻게 고쳤나 — **한도를 늘리되 감시는 세게**

한도를 늘리는 것만으로 초록을 만드는 것은 금지였고, 그렇게 하지 않았다.
**교착을 먼저 고쳤고**(§23.11.6), 그 위에서 시험을 세 군데 고쳤다.

| 무엇 | 예전 | 지금 | 왜 |
| --- | --- | --- | --- |
| 판정 시점 | 30 초에 **한 번** 본다 | **2 초 동안 조용한지** 본다 | 한계 순환은 순간마다 0 이 됐다 안 됐다 한다. 한 번 보는 것은 동전 던지기다 |
| 한도 | 30 초 (근거 없음) | **60 초** (관측 최장 25.92 초의 2.3 배) | 두 기계가 사실상 다른 판을 돈다(§23.11.3). 한도는 관측치가 아니라 그 몇 배여야 한다 |
| 판 | **하나** | 하나 + **흔든 판 셋** | 점 하나로는 성질을 못 지킨다. 이 결함이 그렇게 숨었다 |

**한도를 늘려도 이 시험이 잡는 것은 그대로다.** 고치기 전의 그 판들은 **300 초를 굴려도**
안 멎었고 주기 50 초로 되풀이했다 — 60 초든 30 초든 똑같이 걸린다. 반대로 **감시는 세졌다** —
「순간 0」이 아니라 「2 초 동안 0」을 요구하고, 판을 넷 본다.

**그리고 대개 더 빨리 끝난다.** 예전에는 판이 언제 멎든 1800 프레임을 꽉 채워 굴렸는데,
지금은 조용해지면 거기서 끊는다. 흔한 판은 7 초 언저리다 — **한도는 못 멎는 판에만 든다.**

#### 고친 뒤의 분포 (시작 자리 축, 401 판)
...
### 23.11.8 정리 — 무엇이 사실이고 무엇이 아직 아닌가

| | |
| --- | --- |
| **판정** | **(나) 진짜 교착.** 꼬리가 두꺼운 것이 아니었다 |
| **근거** | 시작 자리를 ±0.5 px 흔든 **298 판 중 16 판(5.4 %)이 60 초 안에 안 멎었고**, 그중 하나는 **300 초를 굴려도** 주기 50 초로 되풀이했다 |
| **CI 와의 연결** | 안 멎은 판의 30 초 시점 `MOVING` 이 **전부 정확히 1** — CI 메시지 `[1] expected to equal [0]` 과 같다 |
| **원인** | `_review_moving` 의 기다림에 **예산이 없었고**, `resume()` 이 최단 기록을 지금 거리로 **되돌렸다** |
| **처방** | 예산을 두 입구가 나눠 쓰고, 기록은 `minf` 로 남긴다. 예산은 **진전**으로만 되돌려 받는다 |
| **고친 뒤** | 같은 축 **401 판 전부 멎는다.** 중앙 7.12 초 · 90 % 9.80 초 · 최대 25.92 초 |
| **인원 축** | 4 ~ 68 명 **65 판 전부 멎는다.** 최대 22.80 초 (48 명) |

#### 아직 아닌 것 — **정직하게 남긴다**

1. **목표 축 · 격자 축은 고친 뒤 다 못 돌렸다.** 자리 축 401 판과 인원 축 65 판까지다.
   두 축은 `tools/measure_grind_tail.gd -- target|grid` 로 언제든 이어서 돌릴 수 있다
2. **꼬리의 근원은 안 고쳤다.** 비스듬히 낀 몸이 `pressed`/`blocker_id` 에 안 잡히는 것
   (앞 절)이 25.92 초짜리 꼬리를 만든다. **범위 밖이라 안 건드렸고**, 다음에 꺼낼 것으로 적어 둔다
3. **리눅스에서 직접 확인하지 못했다.** 이 기계에 리눅스 Godot 이 없다. 여기서 한 것은
   **같은 분포에서 다른 판을 뽑아** 재현한 것이고, CI 의 그 판 자체를 되살린 것이 아니다
