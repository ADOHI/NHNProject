...
**한 판이 닫힌다.** 주둔지에서 원정을 나가고, 판 위에서 턴이 돌고, 판 옆에 렉카 피드가
걸리고, 탈출하면 정산 화면이 뜨고 저장된 뒤 주둔지로 돌아온다.
그 위에 **세계 3000명**(인구·가족·사건·유명세), **레이맨 절차 애니메이션 + 소프트바디**,
...
│   ├── sfx/                    # 효과음 — 사건 → 층 쌓기 → 합성
│   ├── rekka/                  # **렉카 피드 (M6)** — 사건 → 문장 → 화면. 런타임에 모델을 안 부른다
...
│   ├── rekka/                  # 렉카 피드 화면. 판 옆(가로)·판 위(세로)
│   ├── settlement/             # **정산·결과 화면 (M7 · M10)** — 한 판이 여기서 닫힌다
...
| 없는 것 | 없으면 | 어디에 |
| --- | --- | --- |
| **조우 절차** (전투/협상/도주 분기) | NPC 가 장애물로 전락한다(**M5 — 남은 유일한 핵심**) | 조우 **발생 판정까지만** 있다. `core/combat/` 은 시험대뿐 |
| ~~턴 페이즈 해결기~~ | — | ✅ **2026-08-19** (§33) |
| ~~NPC 탐험가의 판 위 행동~~ | — | ✅ **2026-08-19** (§33) |
| ~~렉카 피드 화면~~ | — | ✅ **2026-08-19** (§32) |

**「절대 자르지 않는 여섯」 중 다섯이 섰다.** 남은 하나가 M5 다.

**보류인 채로 남긴 것 둘** — E3(동시 공격) · E5(특수방 조우).
[05](design/05-rules.md) §5.6 이 *"지금 정하면 구현 중에 어차피 뒤집힌다"* 고 적었고
그 판단을 존중했다. 조우 절차를 만들 때 그 표로 돌아간다.
...
| `core/rekka/` | **판 화면에 실제로 걸린다** | 사건이 `MOVED` 하나뿐이라 대부분 이동 기사다 — 턴 시스템이 붙어야 나머지가 산다 |
| `core/combat/` | 대련 시험대 | **조우까지는 왔다** — 그 뒤 전투 분기가 없어 아직 안 불린다 |
